...
EN = {
 "화면표시":"display_scope", "소주제평균포함":"include_topic_mean", "역채점적용":"reverse_scope",
 "구인환산100":"construct_scaled_100", "분모학생수":"enrollment_denominator",
 "발생건수합계":"incident_total", "분모학생수합계":"enrollment_total",
 "발생률산출학교수":"n_rate_schools", "학생수":"enrollment", "학교응답":"has_school",
 "확정상태":"classification_status", "선정상태":"selection_status",
...
DESC = {
 "화면표시":"시계열 / 단년도 / 숨김. 숨김 지표는 화면에 표시하지 않음",
 "소주제평균포함":"Y=구인환산100을 소주제 평균에 포함 / N=제외",
 "역채점적용":"구인평균=구인환산100에서 역채점. 문항별 원값·분포는 원척도 유지",
 "구인환산100":"소주제 평균용 점수. 역문항은 원평균을 역채점 후 100점 환산. 소수 2자리",
 "분모학생수":"학교폭력 문항의 해당 학교·주기 학교DB 학생 수. 1·2·3학년 남녀 학생 수 합계",
 "학생수":"해당 주기 학교DB의 1·2·3학년 남녀 학생 수 합계",
 "발생건수합계":"문항값과 분모가 모두 유효한 학교의 발생 건수 합계",
 "분모학생수합계":"발생건수합계에 포함한 동일 학교 집합의 학생 수 합계",
 "발생률산출학교수":"발생률 분자·분모에 함께 포함된 학교 수",
 "학교응답":"학교 설문 응답 존재 여부(Y/N). 문항별 충돌 결측 여부와는 별개",
 "확정상태":"대분류 체계는 지표 목록 정리 후 확정 예정",
 "선정상태":"헤드라인 후보이며 최종 대표 지표 선정은 보류",
 ...
 "소주제":"소주제 표시명",
 "응답주체":"'학생' / '학부모' / '교사' / '학교'. 학교 지표는 학교가 응답 단위",
 ...
 "표준편차":"표본표준편차",
 "중앙값":"중앙값",
 "환산100":"선택형은 (평균-척도최소)/(척도최대-척도최소)×100. score100은 기존 환산점수 평균",
 "상위2선지비율":"최상위 2개 선지 응답 비율(%). 선택형 지표만",
 "긍정응답률":"긍정코드 응답 비율(%). 이분형 지표만",
 "지출자중앙값":"0 초과 응답자의 중앙값. 금액형 지표만",
 "시계열비교가능":"Y=그대로 비교 / 조건부=주의사항 확인 / N=시계열 제외",
 "통계유형":"likert(선택형) / binary(이분형) / continuous(금액형) / score100(역량) / school_rate(발생률) / school_count(실시 횟수) / excluded",
 "역문항":"Y=구인 평균에서 역채점 / 척도 부적합=구인 평균 제외 / N",
 "var_2021":"1주기 SPSS 원변수명",
 "var_2025":"2주기 SPSS 원변수명",
 "코드":"응답 선지 코드값",
 "라벨":"해당 코드의 의미. 주기별로 표현이 다를 수 있음",
 "값":"응답값 (코드 또는 실수)",
 "결측보정":"다중응답 미선택 보정 또는 학교 중복 통합·충돌 처리 이력. 공란도 범위 검증은 적용됨",
 ...
 "응답자ID":"학생·학부모는 STUID(학부모는 자녀의 학생 ID), 교사는 TID. 집계에는 사용하지 않는다. "
          "동일 주기의 학생·학부모는 같은 값으로 연결되나, 주기가 다르면 서로 다른 사람이다. 학교 설문은 SCHID",
...
TABLES = [
 ...
 ("micro_teacher_2025.csv","micro_teacher_2025","원자료","교사 개인 단위 long (2주기)",
  "집계 재현 및 연도 갱신용"),
 ("micro_school_2021.csv","micro_school_2021","원자료","학교 단위 long (1주기)","학교별 문항당 한 행"),
 ("micro_school_2025.csv","micro_school_2025","원자료","학교 단위 long (2주기)","중복 충돌 결측 처리 후 학교별 문항당 한 행"),
]
